...
outpath='J:\UV, sol og helseeffekter\BILDERogILLUSTRASJONER\Bilder\2016\Skyggeprosjekt_bilder\Fisheye_bearbeidedebilder';%output figs lagres her
im_cal='J:\UV, sol og helseeffekter\BILDERogILLUSTRASJONER\Bilder\2016\Skyggeprosjekt_bilder\Fisheye_originalbilder\DSCN2333.jpg';
rgb = imread(im_cal);
...
I=test_im2.*test_im; %et bilde med verdier 0 utenfor r90 sirkelen, og innenfor er blåverdier >limpix=255, resten 0
clear test_im test_im2

fish_bw_RH_mirror=flipdim(I,2);%speilvendt binærbilde
clear I
...
%Beregn solpos da bildet ble tatt- klokken var her stilt etter norsk sommertid. UTC-må trekke fra 2 timer:
dayIm=dagnr(im_date(1),im_date(2),im_date(3));%dagen fisheyebildene ble tatt i barnehager og skole
[AzIm,ZenIm]=solpos_bj(im_date(1),im_date(2),im_date(3),im_date(4)+1-2,im_date(5),im_date(6),lat,lon);
...
for i=1:numel(pOH)
    ...
    pix_bane(pOH(i))=fish_bw_RH_grayspace(round(yS(pOH(i),dayIm)),round(xS(pOH(i),dayIm)));
    pix_bane_avg(pOH(i))=sunfiltered(round(yS(pOH(i),dayIm)),round(xS(pOH(i),dayIm)));
...
figure %pikselverdier som funksjon av tid på dagen
...
FLATE_cone = porfyri_returner_horisontalkomponenter('clear_sky',clod,350,'snow free',lat,lon,'cone',60,0,'EPP',T_clearsky)
...
FLATE_horizontal = porfyri_returner_horisontalkomponenter('clear_sky',clod,350,'snow free',lat,lon,'horizontal_element',60,0,'EPP',T_clearsky)
